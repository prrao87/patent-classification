...
def preprocess_concurrent(
    texts: List[str], stopwords: Set[str], chunksize: int = 200
) -> List[str]:
    """
...
def transform_data(
    data_file: str = "data.json", stopword_file: str = "stopwords.txt"
) -> pd.DataFrame:
    """
    ...
    df["list_lemmas"] = preprocess_concurrent(df["text"], stopwords)
...
class LinearSVM:
    ...
    def score(self, model: Pipeline, true: pd.Series, pred: pd.Series) -> None:
        return model_performance(true, pred)
...
if __name__ == "__main__":
    ...
    data_df = transform_data(data_file="data.json")
